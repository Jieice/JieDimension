---
title: 水果合成
platform: Web / HTML5
icon: 🍎
status: 在线试玩
date: 2025-10-05
links:
  - text: 全屏游玩
    url: "/JieDimension/assets/games/fruit-merge-game/index.html"
    external: false
---

## 游戏简介

一款轻松有趣的水果合成游戏！通过合并相同的水果来获得更大的水果，挑战最高分！

### 游戏特色

- **简单易上手**：点击屏幕即可开始游戏
- **策略性强**：合理安排水果位置，创造连锁反应
- **休闲放松**：适合碎片时间游玩
- **挑战高分**：不断刷新自己的最高记录

---

## 🎮 在线试玩

### 💻 PC 端试玩（推荐）

<div style="position: relative; padding-bottom: 75%; height: 0; overflow: hidden; max-width: 800px; margin: 2rem auto; background: var(--bg-secondary); border-radius: 12px;">
  <iframe 
    src="/JieDimension/assets/games/fruit-merge-game/index.html"
    style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; border: 2px solid var(--border-color); border-radius: 12px;"
    frameborder="0"
    allowfullscreen>
  </iframe>
</div>

<div style="text-align: center; margin-top: 1.5rem;">
  <a href="/JieDimension/assets/games/fruit-merge-game/index.html" 
     target="_blank" 
     class="btn"
     style="margin: 0.5rem;">
    🎮 全屏模式（推荐）
  </a>
  <a href="/JieDimension/games/" 
     class="btn btn-secondary"
     style="margin: 0.5rem;">
    ← 返回游戏列表
  </a>
</div>

### 📱 手机端游玩

<div style="background: var(--bg-secondary); padding: 1.5rem; border-radius: 8px; margin: 2rem 0; border-left: 4px solid var(--accent-primary);">
  <p style="margin-bottom: 1rem; color: var(--text-secondary);">
    ⚠️ <strong>手机用户注意</strong>：游戏支持触摸操作，但为了最佳体验，建议：
  </p>
  <ol style="color: var(--text-secondary); line-height: 1.8; margin-left: 1.5rem;">
    <li>点击上方 <strong>"全屏模式"</strong> 按钮打开独立页面</li>
    <li>横屏游玩（效果更好）</li>
    <li>确保网络流畅，等待游戏完全加载</li>
  </ol>
  <p style="margin-top: 1rem; color: var(--text-muted); font-size: 0.9rem;">
    💡 如果嵌入窗口无法操作，必须使用全屏模式
  </p>
</div>

<div style="text-align: center; margin: 1.5rem 0;">
  <a href="/JieDimension/assets/games/fruit-merge-game/index.html" 
     target="_blank" 
     class="btn"
     style="font-size: 1.1rem; padding: 1rem 2rem;">
    📱 手机端全屏游玩
  </a>
</div>

---

## 🕹️ 游戏玩法

### 基本规则

1. **投放水果**：点击屏幕投放水果到容器中
2. **合成升级**：两个相同的水果碰撞会合成更大的水果
3. **获得分数**：每次合成都会获得分数
4. **避免溢出**：水果不能超过顶部线，否则游戏结束

### 操作方法

- **投放水果**：鼠标点击 / 屏幕点击
- **暂停游戏**：按 ESC 键 / 点击暂停按钮
- **重新开始**：游戏结束后点击重新开始

### 进阶技巧

- 💡 **预判位置**：观察下一个水果，提前规划
- 💡 **创造连锁**：让多个水果同时合成，获得更高分
- 💡 **保持平衡**：避免一侧堆积过高
- 💡 **战略性等待**：有时候等待比立即投放更好

---

## 🎯 水果进化链

```
🍒 樱桃 → 🍓 草莓 → 🍇 葡萄 → 🍊 橙子 
   ↓
🍋 柠檬 → 🍎 苹果 → 🍐 梨 → 🍑 桃子
   ↓
🍍 菠萝 → 🥥 椰子 → 🍉 西瓜 ← 终极水果！
```

---

## 📊 挑战目标

| 难度 | 目标分数 | 称号 |
|------|---------|------|
| 🌱 新手 | 500 分 | 水果学徒 |
| 🌿 进阶 | 1000 分 | 合成高手 |
| 🌲 高手 | 2000 分 | 水果大师 |
| 🌳 专家 | 5000 分 | 传奇果农 |

**你能达到哪个级别？挑战看看吧！**

---

## 💻 技术实现

- **游戏引擎**：使用 Phaser 3 框架
- **物理引擎**：Matter.js 物理模拟
- **响应式设计**：支持 PC 和移动设备
- **音效系统**：完整的音频管理

---

## 📱 移动端支持

本游戏支持移动设备触摸操作：

- ✅ 支持触摸点击投放水果
- ✅ 自适应屏幕尺寸
- ⚠️ **建议横屏游玩**（体验更好）
- 💡 **使用全屏模式**（避免iframe限制）

### 手机游玩步骤

1. 点击 **"全屏模式"** 按钮
2. 横屏显示手机
3. 点击屏幕投放水果
4. 尽情享受！

### 操作说明（手机）

- **投放水果**：点击/触摸屏幕任意位置
- **暂停游戏**：点击右上角暂停按钮
- **重新开始**：游戏结束后点击重新开始

---

## 🎨 关于这个游戏

这是一款基于经典"合成大西瓜"玩法的 HTML5 游戏，使用纯 Web 技术开发，无需下载即可畅玩。

游戏特别优化了：
- 流畅的物理效果
- 清爽的视觉设计
- 简单易懂的操作
- 快速的加载速度

---

## 🔧 开发信息

- **开发者**：界维互动 (JieDimension)
- **引擎**：Phaser 3 + Matter.js
- **类型**：休闲益智 / 合成游戏
- **首发日期**：2025-10-05

---

<div style="text-align: center; margin: 3rem 0; padding: 2rem; background: var(--bg-secondary); border-radius: 12px;">
  <h3 style="margin-bottom: 1rem;">💬 反馈与建议</h3>
  <p style="color: var(--text-secondary); margin-bottom: 1rem;">
    喜欢这个游戏吗？有什么建议或发现了 Bug？
  </p>
  <p>
    欢迎发送邮件到：
    <a href="mailto:3348149202@qq.com">3348149202@qq.com</a>
  </p>
</div>

---

**享受游戏乐趣！** 🎉🍉

